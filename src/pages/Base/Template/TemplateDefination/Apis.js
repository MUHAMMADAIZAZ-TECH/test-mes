...
export const createTemplate = async ({ body, handleSnackbarOpen, navigate },setConfig) => {
    setConfig({ loading: true, error: null })
    try {
        const response = await api.post("/TemplateDefinition", body);
        if (response.status === 201) {
            setConfig({ loading: false, error: null })
            handleSnackbarOpen("success", "Template definition succesfully!");
            navigate(-1)
        }
    } catch ({ response }) {
...
export const updateTemplate = async ({ body, handleSnackbarOpen, navigate },setConfig) => {
    setConfig({ loading: true, error: null })
    try {
        const response = await api.put(`/Template/${body.tempCode}`, body);
        if (response.status === 200) {
            setConfig({ loading: false, error: null })
            handleSnackbarOpen("success", "Template definition updated succesfully!");
            navigate(-1)
        }
    } catch ({ response }) {
        setConfig({ loading: false, error: response })
...
export const deleteTemplate = async ({ body, fetchFunction, handleSnackbarOpen }) => {
    try {
        ...
        if (response.status === 204) {
            fetchFunction()
            handleSnackbarOpen("success", "Template definition deleted succesfully!");
        }
    } catch ({ response }) {
