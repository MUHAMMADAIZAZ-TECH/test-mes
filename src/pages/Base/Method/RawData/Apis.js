...
export const createRawData = async ({ body, handleSnackbarOpen, navigate }, setConfig) => {
    setConfig({ loading: true, error: null })
    try {
        const response = await api.post("/RawData", body);
        if (response.status === 201) {
            setConfig({ loading: false, error: null })
            handleSnackbarOpen("success", "raw data created succesfully!");
            navigate(-1)
        }
    } catch ({ response }) {
...
export const updateRawData = async ({ body, handleSnackbarOpen, navigate }, setConfig) => {
    setConfig({ loading: true, error: null })
    try {
        const response = await api.put(`/RawData/${body.testDataCode}`, body);
        if (response.status === 200) {
            setConfig({ loading: false, error: null })
            handleSnackbarOpen("success", "raw data updated succesfully!");
            navigate(-1)
        }
    } catch ({ response }) {
        setConfig({ loading: false, error: response })
...
export const deleteRawData = async ({ body, fetchFunction, handleSnackbarOpen }) => {
    try {
        const response = await api.delete(`/RawData/${body.testDataCode}`);
        if (response.status === 204) {
            fetchFunction()
            handleSnackbarOpen("success", "raw data deleted succesfully!");
        }
    } catch ({ response }) {
...
export const GenerateColumns = (handleDeleteClick, handleEditClick) => (
    [
        { field: 'testDataCode', headerName: 'Code', flex: 1 },
        { field: 'testDataDesc', headerName: 'Description', flex: 1 },
