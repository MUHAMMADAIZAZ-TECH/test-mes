...
export const createBatchType = async ({ body, handleSnackbarOpen, navigate },setConfig) => {
    setConfig({ loading: true, error: null })
    try {
        const response = await api.post("/batchType", body);
        if (response.status === 201) {
            setConfig({ loading: false, error: null })
            handleSnackbarOpen("success", "Batch type created succesfully!");
            navigate(-1)
        }
    } catch ({ response }) {
...
export const updateBatchType = async ({ body, handleSnackbarOpen, navigate },setConfig) => {
    setConfig({ loading: true, error: null })
    try {
        const response = await api.put(`/batchType/${body.batchTypeCode}`, body);
        if (response.status === 200) {
            setConfig({ loading: false, error: null })
            handleSnackbarOpen("success", "Batch type updated succesfully!");
            navigate(-1)
        }
    } catch ({ response }) {
        setConfig({ loading: false, error: response })
...
export const deleteBatchType = async ({ body, fetchFunction, handleSnackbarOpen }) => {
    try {
        const response = await api.delete(`/batchType/${body.batchTypeCode}`);
        if (response.status === 204) {
            fetchFunction()
            handleSnackbarOpen("success", "Batch type deleted succesfully!");
        }
    }  catch ({ response }) {
...
export const GenerateColumns = (handleDeleteClick, handleEditClick) => (
    [
        { field: 'batchTypeCode', headerName: 'Code', flex: 1 },
        { field: 'batchTypeDesc', headerName: 'Description', flex: 1 },
