...
export const createClient = async ({ body, handleSnackbarOpen, navigate }, setConfig) => {
    setConfig({ loading: true, error: null })
    try {
        const response = await api.post("/customer", body);
        if (response.status === 201) {
            setConfig({ loading: false, error: null })
            handleSnackbarOpen("success", "Client created succesfully!");
            navigate(-1)
        }
    } catch ({ response }) {
...
export const updateClient = async ({ body, handleSnackbarOpen, navigate }, setConfig) => {
    setConfig({ loading: true, error: null })
    try {
        const response = await api.put(`/customer/${body.custID}`, body);
        if (response.status === 200) {
            setConfig({ loading: false, error: null })
            handleSnackbarOpen("success", "Client updated succesfully!");
            navigate(-1)
        }
    } catch ({ response }) {
        setConfig({ loading: false, error: response })
...
export const deleteClient = async ({ body, fetchFunction, handleSnackbarOpen }) => {
    try {
        const response = await api.delete(`/customer/${body.custID}`);
        if (response.status === 204) {
            fetchFunction()
            handleSnackbarOpen("success", "Client deleted succesfully!");
        }
    } catch ({ response }) {
...
export const GenerateColumns = (handleDeleteClick, handleEditClick) => (
    [
        { field: 'custID', headerName: 'Code', flex: 1 },
