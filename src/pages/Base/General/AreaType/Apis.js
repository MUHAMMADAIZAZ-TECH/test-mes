...
export const createAreaType = async ({ body, handleSnackbarOpen, navigate }, setConfig) => {
    setConfig({ loading: true, error: null })
    try {
        const response = await api.post("/areaType", body);
        if (response.status === 201) {
            setConfig({ loading: false, error: null })
            handleSnackbarOpen("success", "Area type created succesfully!");
            navigate(-1)
        }
    } catch ({ response }) {
...
export const updateAreaTypes = async ({ body, handleSnackbarOpen, navigate }, setConfig) => {
    setConfig({ loading: true, error: null })
    try {
        const response = await api.put(`/areaType/${body.areaTypeCode}`, body);
        if (response.status === 200) {
            setConfig({ loading: false, error: null })
            handleSnackbarOpen("success", "Area type updated succesfully!");
            navigate(-1)
        }
    } catch ({ response }) {
        setConfig({ loading: false, error: response })
...
export const deleteAreaType = async ({ body, handleSnackbarOpen, fetchFunction }) => {
    try {
        const response = await api.delete(`/areaType/${body.areaTypeCode}`);
        if (response.status === 204) {
            fetchFunction()
            handleSnackbarOpen("success", "Area type deleted succesfully!");
        }
    } catch ({ response }) {
...
export const GenerateColumns = (handleDeleteClick, handleEditClick) => (
    [
        { field: 'areaTypeCode', headerName: 'Code', flex: 1 },
        { field: 'areaTypeDesc', headerName: 'Description', flex: 1 },
