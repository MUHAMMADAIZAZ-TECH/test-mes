...
const AssignAreaList = ({ url }) => {
    ...
    const onClickUpdate = (row) => {
        navigate(`${pathname}/edit?sec=${row.sectionCode}&prod=${row.productCode}&bat=${row.batchNo}&tem=${row.tempCode}&stg=${row.stageCode}&uo=${row.unitCode}`);
    };
    const onClickInitiate = (row) => {
        navigate(`/execution/transactions/ingredient-quality-adjustment/edit?sec=${row.sectionCode}&prod=${row.productCode}&bat=${row.batchNo}&tem=${row.tempCode}&stg=${row.stageCode}`);
    };
    return (
